...
class ExplanationService:
    ...
    def explain_transaction(self, df_features: pd.DataFrame, risk_level: str) -> Explanation:
        """
        Computes local SHAP attributions and returns top contributing risk factors.
        """
        self.initialize()
        ...
        if self.explainer is not None:
            try:
                ...
                for idx in top_idx:
                    ...
                    obs_val = df_features[fname].iloc[0]
                    top_factors.append(TopRiskFactor(
                        feature=human_desc,
                        impact=impact,
                        shap_value=round(float(val), 4),
                        feature_value=str(obs_val) if obs_val is not None else "None"
                    ))
